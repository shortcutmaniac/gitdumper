...
function download_item() {
    ...
    if [[ "$objname" =~ /[a-f0-9]{2}/[a-f0-9]{38} ]]; then
        ...
        # Parse output of git cat-file -p $hash. Use grep with regex to extract hashes
        if [[ "$type" != "blob" ]]; then
    ...
    # Parse file for other objects using grep with regex
    hashes+=($(grep -oE "[a-f0-9]{40}" "$target"))
    for hash in "${hashes[@]}"; do
        QUEUE+=("objects/${hash:0:2}/${hash:2}")
    done

    # Parse file for packs using grep with regex
    packs+=($(grep -oE "pack-[a-f0-9]{40}" "$target"))
